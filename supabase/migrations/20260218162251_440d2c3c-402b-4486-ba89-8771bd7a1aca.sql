...
CREATE POLICY "bookings_update_comprehensive"
ON public.bookings
FOR UPDATE
USING (
  is_barbershop_owner(auth.uid(), barbershop_id)
  OR has_role(auth.uid(), barbershop_id, 'attendant'::app_role)
  OR (has_role(auth.uid(), barbershop_id, 'barber'::app_role) AND (barber_id IN (SELECT barbers.id FROM barbers WHERE barbers.user_id = auth.uid())))
